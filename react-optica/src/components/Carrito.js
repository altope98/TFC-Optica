...
class Carrito extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            carrito: this.props.location.state.carrito
        })
    }
    componentWillMount() {
        if (!auth.currentUser) {
    ...
    render() {
        ...
        } else if (this.state.status === 'error' || this.state.carrito.length===0) {
            ...
            return (
                <div id="carrito" className="container-fluid mt-4">
                    ...
                    <div id="boton-procesar" className="col-12 col-md-3 align-self-center">
                        <div className="row justify-content-center">
                        <button className="btn btn-primary"  onClick={() => this.procesarCompra()} >Procesar compra</button>
                        </div>
                    </div>
